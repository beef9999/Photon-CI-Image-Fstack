...
RUN dnf install -y git cmake gcc-toolset-9-gcc-c++ openssl-devel libcurl-devel libaio-devel sudo zstd wget && \
    ...
    cd gflags-2.1.2 && export CXXFLAGS="-fPIC" && cmake -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_SHARED_LIBS=1 -DBUILD_STATIC_LIBS=1 . && make -j 8 && make install && \
    cd .. && rm -rf gflags-2.1.2 && ln -s libgflags.so.2.1.2 /usr/lib/libgflags.so.2.1
